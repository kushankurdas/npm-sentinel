...
/**
 * @param {object} userAllow - optional { suffixes: [], exactHosts: [] }
 */
function mergeAllowlist(userAllow) {
  const base = loadDefaultAllowlist();
  return {
    suffixes: [
      ...(base.suffixes || []),
      ...((userAllow && userAllow.suffixes) || []),
    ],
    exactHosts: [
      ...(base.exactHosts || []),
      ...((userAllow && userAllow.exactHosts) || []),
    ],
  };
}
...
export function runSandbox(opts) {
  ...
  try {
    ...
    const allow = mergeAllowlist(opts.userAllowlist);
